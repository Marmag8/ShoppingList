...
public partial class RecipeGroupView : ContentView
{
    public static readonly BindableProperty CategoryProperty =
        BindableProperty.Create(nameof(Category), typeof(string), typeof(RecipeGroupView), propertyChanged: OnCategoryChanged);

    public static readonly BindableProperty AllRecipesProperty =
        BindableProperty.Create(nameof(AllRecipes), typeof(IEnumerable<RecipeModel>), typeof(RecipeGroupView), propertyChanged: OnAllRecipesChanged);
    ...
    {
        if (bindable is RecipeGroupView v)
        {
    ...
    private static void OnAllRecipesChanged(BindableObject bindable, object oldVal, object newVal)
    {
        if (bindable is RecipeGroupView v)
        {
            if (oldVal is INotifyCollectionChanged oldCollection)
                oldCollection.CollectionChanged -= v.OnSourceCollectionChanged;

            if (newVal is INotifyCollectionChanged newCollection)
                newCollection.CollectionChanged += v.OnSourceCollectionChanged;
    ...
    private void UpdateFiltered()
    {
        ...
        foreach (RecipeModel recipe in AllRecipes.Where(recipe => string.Equals(recipe.Category ?? string.Empty, Category ?? string.Empty, System.StringComparison.OrdinalIgnoreCase)))
